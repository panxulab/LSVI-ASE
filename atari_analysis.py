...
def get_process_result_dict(result, config_idx, mode='Train'):
  result_dict = {
    ...
    'Return (max)': result['Return'][-10:].max(skipna=False) if mode=='Train' else result['Return'][-5:].max(skipna=False)
  }
  return result_dict
...
cfg = {
  ...
  'EMA': True,
  'loc': 'upper left',
  'sweep_keys': ['optimizer/name', 'optimizer/kwargs/noise_scale', 'optimizer/kwargs/gamma', 'optimizer/kwargs/lr', 'optimizer/kwargs/a', 'agent/update_num', 'agent/is_double', 'agent/eps_start'],
