...
class LangevinAdam(Adam):
  def __init__(
    ...
    super(LangevinAdam, self).__init__(params, **defaults)
...
class AntiLangevinAdam(Adam):
  """
  Langevin Adam with anticorrelated noise
  """
  def __init__(
    self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
    weight_decay=0, amsgrad=False,
    ...
    super(AntiLangevinAdam, self).__init__(params, **defaults)
    self.noise_scale = noise_scale
...
def anti_langevin_adam(
  ...
  *,
  ...
  """Functional API that performs Langevin Adam algorithm computation with anticorrelated noise.
  See :class:`~torch.optim.Adam` for details.
  """
  new_noises = []
  for i, param in enumerate(params):
    ...
    grad_perturb = (new_noise - old_noise) / exp_avg_sq.sqrt().add_(eps).sqrt()
    param.add_(noise_scale * math.sqrt(scaled_lr) * grad_perturb)
