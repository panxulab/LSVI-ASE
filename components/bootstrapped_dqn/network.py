...
class smallBootstrappedDQNNet(nn.Module):
  """
  Small Bootstrapped DQN Net.
  """
  def __init__(
    self,
    ...
    self.prior_model = smallEnsembleDQNNet(state_shape, action_shape, device, num_ensemble)
    self.model = smallEnsembleDQNNet(state_shape, action_shape, device, num_ensemble)
...
class smallEnsembleDQNNet(nn.Module):
  def __init__(
    self,
    state_shape: Sequence[int],
    action_shape: Sequence[int],
    ...
    self.head_list = nn.ModuleList([
      nn.Sequential(
        nn.Linear(state_size, 32), nn.ReLU(inplace=True),
