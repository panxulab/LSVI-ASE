...
from components.fgprior_dqn.buffer import FGPriorReplayBuffer as ReplayBuffer
from components.fgprior_dqn.buffer import FGPriorReplayBufferManager as ReplayBufferManager
from components.fgprior_dqn.buffer import FGPriorVectorReplayBuffer as VectorReplayBuffer


class FGPriorCollector(Collector):
  """
  Collector for FGPrior which also stores the first obs of each episode: (o_{1}, o_{t}, a_{t}, o_{t+1}, ...)
  """
  def __init__(
    self,
    policy: BasePolicy,
  ...
  def reset_env(self, gym_reset_kwargs: Optional[Dict[str, Any]] = None) -> None:
    ...
    self.data.obs = obs
    # Set the first obs of each episode
    self.obs_first = obs
    self.data.obs_first = self.obs_first
  ...
  def _reset_env_with_ids(
    ...
    # Set the first obs of each episode
    self.obs_first[local_ids] = obs_reset
    self.data.obs_next[local_ids] = obs_reset
