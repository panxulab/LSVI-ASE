...
from components.fgprior_dqn.collector import FGPriorCollector as Collector
...
class FGPriorDQN(BaseAgent):
  ...
  def __init__(self, cfg):
    ...
    # Set optimizer
    assert self.cfg['optimizer']['name'] in ['LangevinAdam', 'AntiLangevinAdam', 'aSGLD', 'AntiaSGLD', 'pSGLD', 'mSGLD', 'SGLD', 'EulerULMC', 'aULMC'], \
     f"{self.cfg['optimizer']['name']} is not supported for FGPrior DQN."
  ...
  def createNN(self):
    if self.env_name in ['DiabolicalCombLock-v0', 'MountainCar-v0', 'SparseMountainCar-v0', 'NChain-v1']:
      NN = smallDQNNet(
