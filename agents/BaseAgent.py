...
from envs.env import make_atari_env, make_exploration_env, make_gym_env
...
class BaseAgent(object):
  def __init__(self, cfg):
    ...
    # Make envs
    self.envs = dict()
    if self.env_name in ['DiabolicalCombLock-v0', 'SparseMountainCar-v0', 'NChain-v1']:
      self.save_only_last_obs = False
      self.env, self.envs['Train'], self.envs['Test'] = make_exploration_env(
        task = self.env_name,
      ...
      self.save_only_last_obs = False
      self.env, self.envs['Train'], self.envs['Test'] = make_gym_env(
        task = self.env_name,
        agent = self.agent_name,
        seed = self.cfg['seed'],
        training_num = self.cfg['env']['train_num'],
